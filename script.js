(() => {
  ...
  const GAUGE_ARC_LENGTH = 314; // approx π × r(100)
  ...
  function drawTicks() {
    document.querySelectorAll(".gauge-ticks").forEach((g) => {
      ...
      for (let i = 0; i <= 10; i += 2) {
        const angle = Math.PI - (i / 10) * Math.PI; // 180° → 0°
  ...
  // Renders an error message in the error state panel.
  // Note: the HTML error state has only an error-icon and error-copy element —
  // there is no separate error-label element.
  function renderError(copy) {
    errorCopyEl.textContent = copy;
    showState("error");
  }

  // ─── Parse FastAPI / Pydantic 422 errors into field-level messages ────────
  ...
  form.addEventListener("submit", async (e) => {
    ...
    try {
      ...
      renderError(
        `Couldn't connect to the backend at ${backendUrl}. ` +
        "Make sure the server is running (uvicorn main:app --port 8000 --reload) and reachable from this page."
      );
    } finally {
      setSubmitting(false);
    }
